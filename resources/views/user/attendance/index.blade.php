...
@section('content')
    <div class="container my-2 text-muted">
        ...
        <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap">
            ...
            {{-- Filter --}}
            <form method="GET" action="{{ route('attendance.index') }}" class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                {{-- Tahun --}}
                    ...
                    <div class="card border-0 shadow-sm rounded-3 m-0 ">
                        <div class="card-body d-flex justify-content-between align-items-center p-2 flex-wrap">
                            ...
                            <div class="text-end small">
                                <div><span class="text-muted">Masuk:</span> <span
                                        class="fw-semibold">{{ $p->masuk ?? '-' }}</span></div>
                                <div><span class="text-muted">Keluar:</span> <span
                                        class="fw-semibold">{{ $p->keluar ?? '-' }}</span></div>
                                @if ($p->id)
                                    <a href="{{ route('attendance.show', [$p->id, $p->tanggal, $p->type]) }}"
                                        class="btn btn-sm btn-outline-primary rounded-pill mt-1 px-2 py-0 small">
